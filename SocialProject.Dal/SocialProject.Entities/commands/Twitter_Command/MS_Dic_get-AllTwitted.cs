...
{
    public class MS_Dic_get_AllTwitted : BaseCommand, ICommand
    {
        public MS_Dic_get_AllTwitted(LogManager log) : base(log) { }

        public (string, object) Run(params object[] arg)
        {
            try
            {
                Log.LogEvent(@"Event: Entities \ commands \ MS_Dic_get_AllTwitted \ Run, ran Successfully - ");

                MainManager.Instance.twitterManager.ShowTweetsListFromDB();
                string responseMessage = JsonConvert.SerializeObject(MainManager.Instance.twitterManager.getTweetsList);
